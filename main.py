...
def insertintotable():
    ...
    def LSTM_ALGO(df):
        ...
        error_lstm=np.sqrt(np.mean(((predictions- y_test)**2)))
        ...
        plt.ylabel('Close Pice INR (₹)', fontsize=18)
        plt.plot(valid[['Close', 'Predictions']])
        plt.legend(['Val', 'Predictions'], loc='lower right')
        plt.savefig('static/LSTM.png')

        rel_quote = yf.download(tickers='TATAMOTORS.NS', period='max', interval='1d')
        ...
        lstm_pred = np.asscalar(lstm_pred)
    ...
    def recommending(df, global_polarity,today_stock, lstm_pred):
        if today_stock.iloc[-1]['Close'] < lstm_pred:
            if global_polarity > 0:
                idea="RISE"
                decision="BUY"
                print()
